...
public final class HallOfMemories {
    ...
    public static boolean isMemoryJarFull(int slot) {
        return InventoryQuery.newQuery(93)
                .slot(slot)
                .results()
                .first()
    ...
    public static boolean hasFullJars(int count) {
        ResultSet<Item> results = InventoryQuery.newQuery(93).itemId(42899).results();
        int c = 0;
    ...
    public static boolean isMemoryBotRecovered(MemoryBotData memoryBot) {
        ResultSet<Item> results = InventoryQuery.newQuery(93).category(4322).results();
        if(results.isEmpty()) {
            return false;
        }
